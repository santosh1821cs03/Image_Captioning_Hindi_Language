...
class CaptionGenerator(object):
    ...
    def masking(self,scores, sequence_lengths, score_mask_value=tf.constant(-np.inf)):
        score_mask = tf.sequence_mask(sequence_lengths, maxlen=tf.shape(scores)[0])
    ...
    def _attention_layer(self, features, features_proj, h, reuse=False):
        with tf.variable_scope('attention_layer', reuse=reuse):
            ...
            out_att = self.masking(out_att, self.T, tf.constant(-1e15, dtype=tf.float32))
    ...
    # def _attention_layer(self,inputs, att_dim, sequence_lengths, reuse=False):
    ...
    #         outputs = tf.reduce_sum(inputs * tf.expand_dims(alpha, 2), axis=1, name='context')
